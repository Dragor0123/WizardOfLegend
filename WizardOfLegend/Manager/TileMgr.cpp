...
void CTileMgr::Load_Tile(const string& _strImageKey)
{
	...
	while (true)
	{
		...
		else if (_strImageKey == "PlazaTile" || _strImageKey == "EarthBossTile") {
		...
		//if (tInfo.fX >= 859.f && tInfo.fX <= 2397.f &&
		//	tInfo.fY >= 338.f && tInfo.fY <= 1437.f)
		//{
		//	dynamic_cast<CTile*>(pObj)->Set_DrawID_Row(10);
		//	dynamic_cast<CTile*>(pObj)->Set_DrawID_Col(8);
		//	dynamic_cast<CTile*>(pObj)->Set_Option(TILEENUM::OPT_MOVE);
		//}
